...
mod tests {
    ...
    use crate::{
        config::NtsPoolKeConfig,
        nts::{AlgorithmDescription, FixedKeyRequest, KeyExchangeResponse, ProtocolId, MAX_MESSAGE_SIZE},
        pool_ke::NtsPoolKe,
        servers::{Server, ServerConnection, ServerManager}, util::BufferBorrowingReader,
    };
    ...
    async fn test_keyexchange_basic() {
        ...
        conn.write_all(&[0x80, 1, 0, 2, 0, 0, 0x80, 4, 0, 2, 0, 0, 0x80, 0, 0, 0])
            ...
            .unwrap();
        let mut buf = [0u8; MAX_MESSAGE_SIZE as _];
        let response = KeyExchangeResponse::parse(&mut BufferBorrowingReader::new(&mut conn, &mut buf)).await.unwrap();
        conn.shutdown().await.unwrap();

        let mut buf = [0u8; MAX_MESSAGE_SIZE as _];
        #[allow(clippy::await_holding_lock)]
        let timesource_request =
            FixedKeyRequest::parse(&mut BufferBorrowingReader::new(manager.inner.written.lock().unwrap().as_slice(), &mut buf))
                .await
                .unwrap();
    ...
    async fn test_keyexchange_respects_client_prioritization_1() {
        ...
        let mut buf = [0u8; MAX_MESSAGE_SIZE as _];
        #[allow(clippy::await_holding_lock)]
        let request = FixedKeyRequest::parse(&mut BufferBorrowingReader::new(manager.inner.written.lock().unwrap().as_slice(), &mut buf))
            .await
            .unwrap();
